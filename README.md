...
```bash
cd ~
git clone https://github.com/QUAR-Network/QUARMasternodeSetup
```
...
**6) Prepare your Hot Wallet and start the new masternode**. In this step you will introduce your new masternode to the QUAR network by issuing a masternode start command from your wallet, which will broadcast information proving that
the collateral for this masternode is secured in your wallet. Without this step your new masternode will function as a regular QUAR node (wallet) and will not yield any rewards. Usually you keep your Hot Wallet on your Windows machine where you securely store your funds for the MN collateral.
...
There are two ways to edit __masternode.conf__. The easiest way is to open the file from within the wallet app (Tools -> Open Masternode Configuration File). Optionally, you can open it from the wallet data folder directly by navigating to the %appdata%/roaming/QUARcore. Just hit Win+R, paste %appdata%/roaming/QUARcore, hit Enter and then open **masternode.conf** with Notepad for editing. 
...
__publicipaddress:39393__ - this must be your masternode public IP address, which is usually the IP address of your VPS, accessible from the Internet. The new script (v1.1) will detect your IP address automatically. The __:39393__ suffix is the predefined and fixed TCP port which is being used in QUAR network for node-to-node and wallet-to-node communications. This port needs to be opened on your VPS server firewall so that others can talk to your masternode. The setup script takes care of it. NOTE: some VPS service providers may have additional firewall on their network which you may need to configure to open TCP port  39393. Vultr does not require this.

__masternodeprivatekey__ - this is your masternode private key which script will generate automatically. Each masternode will use its own unique private key to maintain secure communication with your Hot Wallet. You will have to generate a new key for each masternode you are setting up. Only your masternode and your hot wallet will be in possession of this private key. In case if you will need to change this key later for some reason, you will have to update it in your __masternode.conf__ in Hot Wallet as well as in the QUAR.conf in data directory on the masternode VPS.

__output-tx-ID__ - this is your collateral payment Transaction ID which is unique for each masternode. It can be easily located in the transaction details (Transactions tab) or in the list of your **masternode outputs**. This TxID also acts as unique masternode ID on the QUAR network.
...
Once the wallet is fully synchronized and your masternode setup script on VPS has finished its synchronization with the network, you can **issue a start broadcast** from your hot wallet to tell the others on QUAR network about your new masternode.
...
```bash
sudo tail -f ~/.QUARcore/debug.log
```

And for those who wonder what does **QUAR.conf** file looks like for a typical masternode which the setup script generates, here's an example below...

Note that both, the __externalip__ should match the IP address and __masternodeprivkey__ should math the private key in your  __masternode.conf__ of your hot wallet in order for the masternode to function properly. If any of these two parameters change, they must be changed in both, the QUAR.conf file on the masternode VPS (located in /root/.QUARcore directory) and masternode.conf on Hot Wallet PC (located in %appdata%/QUARcore folder).

Example: 

**nano /root/.QUARcore/QUAR.conf**
...
The script adds a cron job which starts QUARd daemon upon reboot. Try restarting your VPS server (just type reboot in Linux console) and see if your masternode comes back online automatically in a few minutes. Log back in using PuTTY and run the following command to monitor your masternode status:

```
watch -n 10 'QUAR-cli masternode status && QUAR-cli mnsync status'
```
...
===========================================================================
Sync Status:
# QUAR-cli -datadir=/root/.QUARcore mnsync status
{
  "AssetID": 999,
  "AssetName": "MASTERNODE_SYNC_FINISHED",
  "Attempt": 0,
...
# QUAR-cli -datadir=/root/.QUARcore getinfo
{
  "version": 2000001,
...
Example: nodemon.sh 10 22 will run every 10 seconds and query QUARd in /root/.QUARcore22
...
...please donate QUAR to: **RCdYg5yq3YfymwrZi8EMBSFHxcwR7acniS**

or just come to our pool to mine QUAR: https://fasterpool.com
